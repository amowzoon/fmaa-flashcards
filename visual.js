...
indirect:{title:"Indirect method: add or subtract?",buckets:["Add to net income","Subtract","No adjustment"],items:[["Depreciation expense","Add to net income"],["Gain on sale of equipment","Subtract"],["Increase in accounts receivable","Subtract"],["Decrease in inventory","Add to net income"],["Increase in accounts payable","Add to net income"],["Decrease in accrued wages","Subtract"],["Loss on early debt extinguishment","Add to net income"],["Interest expense paid in cash","No adjustment"],["Amortization of bond discount","Add to net income"],["Increase in prepaid insurance","Subtract"],["Purchase of a machine for cash","No adjustment"],["Impairment loss","Add to net income"]]},
control:{title:"Which control duty is this?",buckets:["Authorization","Recordkeeping","Custody","Reconciliation"],items:[["Approving customer credit limits","Authorization"],["Posting payments to customer accounts","Recordkeeping"],["Opening the mail and holding checks","Custody"],["Comparing bank statement to cash ledger","Reconciliation"],["Signing purchase orders","Authorization"],["Counting physical inventory against records","Reconciliation"],["Keeping the securities in the safe","Custody"],["Preparing journal entries","Recordkeeping"]]},
costs:{title:"Inventory cost: capitalize or expense?",buckets:["Inventory (product) cost","Period cost"],items:[["Freight-in","Inventory (product) cost"],["Freight-out to customers","Period cost"],["Factory overhead","Inventory (product) cost"],["Officers' salaries","Period cost"],["Insurance in transit","Inventory (product) cost"],["Advertising","Period cost"],["Direct labor","Inventory (product) cost"],["General interest on borrowings","Period cost"]]},
revenue:{title:"Revenue: over time or point in time?",buckets:["Over time","Point in time"],items:[["Three-year technical support contract","Over time"],["Pizza sold at the counter","Point in time"],["Building on the customer's land","Over time"],["Custom machine, no alternative use, right to payment for work done","Over time"],["Warehouse with refundable deposit and payment only on completion","Point in time"],["Bill-and-hold furniture meeting all criteria","Point in time"],["Monthly lawn service paid annually in advance","Over time"],["Consigned goods sold by the dealer","Point in time"]]},
mixed:{title:"Mixed set (interleaved)",buckets:null,items:null}};
let sim={sel:null,placed:{},set:null};
function simSet(){if(pick.sim!=="mixed")return SIMS[pick.sim];if(!sim.set){const keys=Object.keys(SIMS).filter(k=>k!=="mixed");const k=shuffle(keys.slice()).slice(0,2);const it=[];k.forEach(x=>SIMS[x].items.forEach(i=>it.push([i[0],i[1],x])));sim.set={title:"Mixed: "+k.map(x=>SIMS[x].title).join(" + "),buckets:[].concat(...k.map(x=>SIMS[x].buckets)),items:shuffle(it).slice(0,12)};}return sim.set;}
function drawSim(){
  ...
  return `<div class="vinfo"><div class="vt">${esc(s.title)}</div><div class="meta">Tap an item, then its bucket (drag also works). ${Object.keys(placed).length}/${items.length} placed · ${score} correct</div></div>
  ...
  <div class="controls"><button id="vreset">Reset</button></div>${method("categorisation with immediate feedback"+(pick.sim==="mixed"?", interleaved across topics":""))}`;
}
